...
check("NEW section shown", "NEW SINCE LAST RUN (2)" in out1)
check("no SEEN BEFORE section", "SEEN BEFORE" not in out1)
...
check("SEEN BEFORE section shown", "SEEN BEFORE (2)" in out2)
check("no NEW section", "NEW SINCE LAST RUN" not in out2)
...
check("the new one is named", "Product Operations Lead" in out3.split("SEEN BEFORE")[0])
check("other two are returning", "SEEN BEFORE (2)" in out3)
